...
public class DynamicEndpointSnitch extends AbstractEndpointSnitch implements LatencySubscribers.Subscriber, DynamicEndpointSnitchMBean
{
     ...
     */
    public void receiveTiming(InetAddressAndPort address, long latency, TimeUnit unit, LatencyMeasurementType measurementType)
    {
        if (measurementType == LatencyMeasurementType.IGNORE)
           return;

        AnnotatedMeasurement sample = samples.get(address);
        long latencyMicros = unit.toMicros(latency);
