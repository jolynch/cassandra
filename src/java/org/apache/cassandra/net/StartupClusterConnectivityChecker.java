...
public class StartupClusterConnectivityChecker
{
    ...
    public void execute(Set<InetAddressAndPort> peers)
    {
        ...
        logger.info("waiting until fewer than [{},{},{}] peers in [local, each, all] dcs are down or do not have connections established; max time to wait = {} seconds",
        ...
        while (checkStatus(peers, localDC, connectedCounts, startNanos, expirationNanos < System.nanoTime(), completedRounds) == State.CONTINUE)
        {
            completedRounds++;
    ...
    State checkStatus(Set<InetAddressAndPort> peers, String localDC, Map<InetAddressAndPort, AtomicInteger> connectedCounts, final long startNanos, boolean beyondExpiration, final int completedRounds)
    {
        ...
        List<Long> downCounts = new ArrayList<>(downPeersPerDC.values());
        downCounts.add(0L);
        ...
        List<Long> disconnectedCounts = new ArrayList<>(notConnectedPeersPerDC.values());
            ...
            (currentDownAllDCS <= downCountAllDCS && currentDisconnectedAllDCS <= downCountAllDCS))
        {
            logger.info("after {} milliseconds, found [{},{},{}]/{} of peers still marked down, " +
                        ...
                        "all of which are below the desired threshold of [local, any dc, all dcs] <= [{},{},{}]",
                        ...
                        expressBound(downCountLocalDC), expressBound(downCountPerDC), expressBound(downCountAllDCS));
    ...
    private String expressBound(int bound) {
