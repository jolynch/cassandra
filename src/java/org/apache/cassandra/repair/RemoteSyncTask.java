...
public class RemoteSyncTask extends SyncTask
{
    ...
    public RemoteSyncTask(RepairJobDesc desc, InetAddress r1, InetAddress r2, List<Range<Token>> differences)
    {
        super(desc, r1, r2, differences);
    }

    protected void startSync(List<Range<Token>> differences)
    {
        InetAddress local = FBUtilities.getBroadcastAddress();
        SyncRequest request = new SyncRequest(desc, local, endpoint1, endpoint2, differences);
    ...
    public void syncComplete(boolean success)
    {
        ...
        {
            setException(new RepairException(desc, String.format("Sync failed between %s and %s", endpoint1, endpoint2)));
        }
    }
}
