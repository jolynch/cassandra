...
public class DynamicEndpointSnitchTest
{
    ...
    private static void setScores(DynamicEndpointSnitch dsnitch, int rounds, List<InetAddressAndPort> hosts, Integer... scores) throws InterruptedException
    {
        for (int round = 0; round < rounds; round++)
        {
            for (int i = 0; i < hosts.size(); i++)
                dsnitch.receiveTiming(hosts.get(i), scores[i], MILLISECONDS, LatencyMeasurementType.READ);
        }
    ...
    public void testLatencyProbeNeeded()
    {
        ...
        dsnitch.receiveTiming(hosts[1], 20, MILLISECONDS, LatencyMeasurementType.READ);
        dsnitch.receiveTiming(hosts[2], 10, MILLISECONDS, LatencyMeasurementType.READ);
        dsnitch.receiveTiming(hosts[3], 1000, MILLISECONDS, LatencyMeasurementType.READ);
        dsnitch.receiveTiming(hosts[4], 1000, MILLISECONDS, LatencyMeasurementType.READ);
        dsnitch.updateScores();
    ...
    public void testNoLatencyProbesNeeded()
    {
        ...
        dsnitch.receiveTiming(hosts[1], 20, MILLISECONDS, LatencyMeasurementType.PROBE);
        dsnitch.receiveTiming(hosts[2], 10, MILLISECONDS, LatencyMeasurementType.PROBE);
        dsnitch.receiveTiming(hosts[3], 1000, MILLISECONDS, LatencyMeasurementType.PROBE);
        dsnitch.updateScores();
